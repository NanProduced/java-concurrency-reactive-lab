...
| DT-005 | 异步任务超时时间配置 | Lab-04 | ✅ 已记录 |
| DT-006 | 异步方案选择 | Lab-05 | ⏳ 待补充 |
| DT-007 | 背压策略选择 | Lab-07 | ⏳ 待补充 |
| DT-008 | GC 算法选择 | Lab-09 | ⏳ 待补充 |
| DT-009 | 响应式框架选择 | Lab-11 | ⏳ 待补充 |
...
**应用场景**: 为 CompletableFuture 异步任务、HTTP 外部调用、数据库查询等设置合理的超时时间
**更新日期**: 2025-01-18（Lab-04）
...
**理由**:
- P99 覆盖 99% 的正常请求
- 1.5 倍系数留出网络抖动和负载波动的缓冲

#### 步骤 2: 根据任务类型调整

| 任务类型 | 推荐范围 | 说明 |
|---------|---------|------|
| 内部服务 | 500ms - 1s | 同一机房，网络稳定 |
| 外部 HTTP API | 2s - 5s | 跨网络，不可控因素多 |
| 数据库查询 | 1s - 3s | 依赖查询复杂度 |
| 缓存读取 | 100ms - 200ms | 内存操作，应该极快 |
| 消息队列 | 5s - 10s | 允许一定延迟 |

**使用规则**:
- 如果基准超时在推荐范围内 → 使用基准超时
- 如果基准超时 < 推荐下限 → 使用推荐下限（避免过短）
- 如果基准超时 > 推荐上限 → 优先优化服务，而非放大超时

#### 步骤 3: 根据重要性微调

| 重要性 | 调整系数 | 说明 |
|-------|---------|------|
| 核心服务 | × 1.2 | 容忍更长等待，降低误杀 |
| 重要服务 | × 1.0 | 使用基准值 |
| 可选服务 | × 0.8 | 快速失败，节省资源 |
...
**理由**:
- 超时时间应留出 20% 缓冲给聚合、处理、网络传输
- 如果验证失败，需要重新评估 SLA 或优化服务性能

#### 步骤 5: 降级策略配置

根据超时后的处理方式选择：

**orTimeout**: 超时抛异常，适合核心服务
```java
.orTimeout(最终超时, TimeUnit.MILLISECONDS)
```

**completeOnTimeout**: 超时返回默认值，适合可选服务
```java
.completeOnTimeout("默认值", 最终超时, TimeUnit.MILLISECONDS)
```
...
**决策**:
1. 基准超时 = 1200ms × 1.5 = 1800ms
2. 任务类型: 1800ms 在 2s-5s 范围内 → 调整为 2000ms（取推荐下限）
3. 重要性微调: 2000ms × 0.8 = 1600ms
4. SLA 验证: 1600ms < 3000ms × 0.8 = 2400ms ✅
5. **推荐配置**: 1600ms + completeOnTimeout

```java
CompletableFuture<String> recommendationFuture = CompletableFuture
    .supplyAsync(() -> recommendationService.fetchRecommendations())
    .completeOnTimeout("默认推荐列表", 1600, TimeUnit.MILLISECONDS);
```
...
**决策**:
1. 基准超时 = 800ms × 1.5 = 1200ms
2. 任务类型: 1200ms 在 1s-3s 范围内 → 使用 1200ms
3. 重要性微调: 1200ms × 1.2 = 1440ms
4. SLA 验证: 1440ms < 2000ms × 0.8 = 1600ms ✅
5. **推荐配置**: 1440ms + orTimeout + 异常恢复

```java
CompletableFuture<UserData> userFuture = CompletableFuture
    .supplyAsync(() -> userRepository.findById(userId))
    .orTimeout(1440, TimeUnit.MILLISECONDS)
    .exceptionally(ex -> {
        log.error("用户查询超时", ex);
        throw new ServiceException("用户数据获取失败");
    });
```
...
1. **基于实测数据**: 不要拍脑袋，使用实际的 P99 数据
2. **留出缓冲**: 至少 1.5 倍的安全系数
3. **SLA 验证**: 确保超时时间不影响 SLA 承诺
4. **监控告警**: 设置超时率和误杀率告警
5. **定期复审**: 每季度审查超时配置的合理性

### 快速参考表

| 任务类型 | 推荐超时 | 降级策略 | 适用场景 |
|---------|---------|---------|---------|
| 内部服务 | 500ms-1s | orTimeout | 核心服务 |
| 外部 API | 2s-5s | completeOnTimeout | 可选服务 |
| 数据库查询 | 1s-3s | orTimeout + 重试 | 核心数据 |
| 缓存读取 | 100ms-200ms | completeOnTimeout | 性能优化 |
| 消息队列 | 5s-10s | orTimeout | 异步处理 |

### 参考资源

- Lab-04 TimeoutCancellationDemo: 完整的超时控制演示
- 《微服务设计模式》: 超时与重试
- Google SRE: 错误预算与 SLA
- lab-04-completablefuture/README.md: 超时控制章节

---

## 待补充的决策树

### DT-006: 异步方案选择 (Lab-05)
决策条件:
- 是否需要 Java 8+ 兼容性?
- 是否涉及多个依赖的异步操作?
- 是否需要背压支持?
